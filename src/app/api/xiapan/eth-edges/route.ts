// /api/xiapan/eth-edges
//
// V0.72 W2 Day 2 · ETH 二元期权公允价 + 跨期限 + 跨平台 (复用 BTC 引擎)
//
// 覆盖 Kalshi ETH series ·
//   KXETHD / KXETHW / KXETHMM / KXETHH
...
async function fetchEthCloses(days: number): Promise<number[]> {
  try {
    const end = new Date();
...
async function fetchKalshiEthSeries(series: string): Promise<KalshiMarket[]> {
  try {
...
function parseEthTicker(m: KalshiMarket): ParsedTicker | null {
  const seriesMatch = m.ticker.match(/^(KXETH[A-Z]{1,3})/);
...
const SERIES_TO_SCAN = ["KXETHD", "KXETHW", "KXETHMM", "KXETHH"];
...
export async function GET() {
  ...
  if (!spot || history30.length < 10) {
    return NextResponse.json(
      { ok: false, error: "ETH spot or history unavailable", spot, history_n: history30.length },
      { status: 200 }
    );
  }
  ...
  const allMarkets = (await Promise.all(SERIES_TO_SCAN.map(fetchKalshiEthSeries))).flat();
  const edges: EthEdge[] = [];
  const bsSignals: Signal[] = [];

  for (const m of allMarkets) {
    const parsed = parseEthTicker(m);
    ...
    if (Math.abs(edge_pp) >= 5 && vol_24 >= 200) {
      ...
      signal = {
        ...
        predicted_p: fair_p,
        confidence: 0.60,
        reason: `BS ETH fair=${(fair_p * 100).toFixed(1)}% vs ${(market_p * 100).toFixed(0)}% · σ=${(sigma * 100).toFixed(0)}% · T=${T_hours.toFixed(1)}h`,
  ...
  const byStrike = new Map<string, EthEdge[]>();
  ...
  for (const [, group] of byStrike) {
    ...
    for (let i = 1; i < group.length; i++) {
      ...
      crossTenorSignals.push({
        source: "eth-cross-tenor",
        ticker: buyTarget.ticker,
        direction,
        predicted_p: buyTarget.fair_p,
        confidence: 0.62,
        reason: `跨期限 ETH ${a.series} ${a.market_p.toFixed(2)} vs ${b.series} ${b.market_p.toFixed(2)} 同 strike $${a.strike}`,
        ts: new Date().toISOString(),
  ...
  try {
    const polyAll = await fetchPolyMarkets({ search: "ETH", limit: 100 });
    const polyAll2 = await fetchPolyMarkets({ search: "Ethereum", limit: 100 });
